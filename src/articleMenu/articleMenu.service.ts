...
export class articleMenuService {
    ...
    async editArticle(params): Promise<any> {
        ...
        return await this.ArtcleMenuRepository.update(params.id, {
            pid: params.pid,
            menuname: params.menuname,
            remork: params.remork,
        }).then(res => {
    ...
    async getArticleMenuList(): Promise<any> {
        const artList = await this.ArtcleMenuRepository.query(`SELECT id,pid,menuname,sort,remork,cdate FROM artcle_menu order by sort`)
